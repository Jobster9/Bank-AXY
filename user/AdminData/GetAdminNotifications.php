...
function GetNotifications()
{
  ...
  $userIDsString = implode(",", array_map(function ($id) use ($pdo) {
    return $pdo->quote($id); }, $userIDs));
  ...
  $auditIDString = implode(",", array_map(function ($aid) use ($pdo) {
    return $pdo->quote($aid); }, $auditIDs));
